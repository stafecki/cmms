...
export const getLocationById = async (id: string): Promise<LocationWithChildren> => {
...
export const updateLocation = async (
    ...
    if (input.parentId) {
        if (input.parentId === id) {
            throw new HTTPException(400, { message: "Location cannot be its own parent" })
        }
        const parent = await prisma.location.findUnique({
            where: { id: input.parentId }
        })
...
export const deleteLocation = async (id: string): Promise<void> => {
    ...
    if (location.children.length > 0) {
        throw new HTTPException(400, { message: "Cannot delete location with children" })
    }

    if (location.machines.length > 0) {
        throw new HTTPException(400, { message: "Cannot delete location with assigned machines" })
    }
