...
const seedLocations = async () => {
    ...
    const plant = await prisma.location.upsert({
        ...
        update: {},
        create: { id: 'seed-plant-001', name: 'Zakład Główny', type: LocationType.PLANT }
    })

    const hallA = await prisma.location.upsert({
        where: { id: 'seed-hall-001' },
        update: {},
        create: { id: 'seed-hall-001', name: 'Hala A – Produkcja', type: LocationType.HALL, parentId: plant.id }
    })

    const hallB = await prisma.location.upsert({
        where: { id: 'seed-hall-002' },
        update: {},
        create: { id: 'seed-hall-002', name: 'Hala B – Montaż', type: LocationType.HALL, parentId: plant.id }
    })

    const line1 = await prisma.location.upsert({
        where: { id: 'seed-line-001' },
        update: {},
        create: { id: 'seed-line-001', name: 'Linia produkcyjna 1', type: LocationType.LINE, parentId: hallA.id }
    })

    const line2 = await prisma.location.upsert({
        where: { id: 'seed-line-002' },
        update: {},
        create: { id: 'seed-line-002', name: 'Linia produkcyjna 2', type: LocationType.LINE, parentId: hallA.id }
    })

    const line3 = await prisma.location.upsert({
        where: { id: 'seed-line-003' },
        update: {},
        create: { id: 'seed-line-003', name: 'Linia montażowa 1', type: LocationType.LINE, parentId: hallB.id }
    })

    await prisma.location.upsert({
        where: { id: 'seed-station-001' },
        update: {},
        create: { id: 'seed-station-001', name: 'Stanowisko CNC', type: LocationType.STATION, parentId: line1.id }
    })

    await prisma.location.upsert({
        where: { id: 'seed-station-002' },
        update: {},
        create: { id: 'seed-station-002', name: 'Stanowisko spawalnicze', type: LocationType.STATION, parentId: line2.id }
    })
...
const seedInventory = async () => {
    ...
    const categories = await Promise.all([
        prisma.partCategory.upsert({ where: { name: 'Łożyska' }, update: {}, create: { name: 'Łożyska' } }),
        prisma.partCategory.upsert({ where: { name: 'Uszczelnienia' }, update: {}, create: { name: 'Uszczelnienia' } }),
        prisma.partCategory.upsert({ where: { name: 'Paski i łańcuchy' }, update: {}, create: { name: 'Paski i łańcuchy' } }),
        prisma.partCategory.upsert({ where: { name: 'Filtry' }, update: {}, create: { name: 'Filtry' } }),
        prisma.partCategory.upsert({ where: { name: 'Elektronika' }, update: {}, create: { name: 'Elektronika' } }),
        prisma.partCategory.upsert({ where: { name: 'Narzędzia' }, update: {}, create: { name: 'Narzędzia' } })
    ])

    const [bearings, seals, belts, filters, electronics, tools] = categories

    const parts = await Promise.all([
        prisma.part.upsert({ where: { qrCode: 'QR-001' }, update: {}, create: { categoryId: bearings.id, name: 'Łożysko SKF 6205', stockQuantity: 15, reorderPoint: 5, unitPrice: 45.99, qrCode: 'QR-001' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-002' }, update: {}, create: { categoryId: bearings.id, name: 'Łożysko FAG 6308', stockQuantity: 8, reorderPoint: 3, unitPrice: 89.50, qrCode: 'QR-002' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-003' }, update: {}, create: { categoryId: seals.id, name: 'Uszczelnienie simmering 30x52x10', stockQuantity: 20, reorderPoint: 8, unitPrice: 12.50, qrCode: 'QR-003' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-004' }, update: {}, create: { categoryId: seals.id, name: 'O-ring NBR 50x3', stockQuantity: 2, reorderPoint: 10, unitPrice: 2.30, qrCode: 'QR-004' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-005' }, update: {}, create: { categoryId: belts.id, name: 'Pasek klinowy SPB-1800', stockQuantity: 6, reorderPoint: 2, unitPrice: 35.00, qrCode: 'QR-005' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-006' }, update: {}, create: { categoryId: filters.id, name: 'Filtr oleju hydraulicznego HF35', stockQuantity: 3, reorderPoint: 3, unitPrice: 78.00, qrCode: 'QR-006' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-007' }, update: {}, create: { categoryId: filters.id, name: 'Filtr powietrza AF25', stockQuantity: 12, reorderPoint: 4, unitPrice: 42.00, qrCode: 'QR-007' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-008' }, update: {}, create: { categoryId: electronics.id, name: 'Czujnik indukcyjny PNP M12', stockQuantity: 5, reorderPoint: 2, unitPrice: 125.00, qrCode: 'QR-008' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-009' }, update: {}, create: { categoryId: electronics.id, name: 'Przekaźnik Schneider RXM4AB1P7', stockQuantity: 10, reorderPoint: 3, unitPrice: 65.00, qrCode: 'QR-009' } }),
        prisma.part.upsert({ where: { qrCode: 'QR-010' }, update: {}, create: { categoryId: tools.id, name: 'Klucz dynamometryczny 40-200Nm', stockQuantity: 3, reorderPoint: 1, unitPrice: 320.00, qrCode: 'QR-010' } })
    ])

    console.log(`✅ Seeded ${categories.length} categories and ${parts.length} parts`)
    return parts
}
...
const seedWorkOrders = async (users: any[], machines: any[], parts: any[]) => {
    ...
    const wo1 = await prisma.workOrder.create({
        data: {
            ...
            description: 'Wrzeciono wydaje głośny dźwięk podczas pracy. Konieczna wymiana łożyska.',
    ...
    await prisma.workOrderPart.create({
        data: { workOrderId: wo1.id, partId: parts.find(p => p.qrCode === 'QR-002').id, quantity: 1 }
    })

    await prisma.workOrderMessage.createMany({
        data: [
            { workOrderId: wo1.id, userId: tech1.id, content: 'Rozpoczynam diagnostykę wrzeciona.', sentAt: new Date('2026-03-10T08:15:00') },
            { workOrderId: wo1.id, userId: tech1.id, content: 'Łożysko FAG 6308 wymienione. Maszyna działa poprawnie.', sentAt: new Date('2026-03-10T12:20:00') },
            { workOrderId: wo1.id, userId: manager.id, content: 'Zlecenie zatwierdzone. Dobra robota.', sentAt: new Date('2026-03-10T12:45:00') }
        ]
    })
    ...
    const wo2 = await prisma.workOrder.create({
        data: {
            ...
            description: 'Zauważono wyciek oleju hydraulicznego z siłownika głównego.',
    ...
    await prisma.workOrderPart.createMany({
        data: [
            { workOrderId: wo2.id, partId: parts.find(p => p.qrCode === 'QR-003').id, quantity: 2 },
            { workOrderId: wo2.id, partId: parts.find(p => p.qrCode === 'QR-004').id, quantity: 5 }
        ]
    })

    await prisma.workOrderMessage.createMany({
        data: [
            { workOrderId: wo2.id, userId: tech2.id, content: 'Wyciek potwierdzony. Zamawiamy uszczelnienia.', sentAt: new Date('2026-03-15T09:30:00') },
            { workOrderId: wo2.id, userId: tech2.id, content: 'Uszczelnienia wymienione. Brak wycieku.', sentAt: new Date('2026-03-15T13:45:00') }
        ]
    })
    ...
    await prisma.workOrderPart.create({
        data: { workOrderId: wo3.id, partId: parts.find(p => p.qrCode === 'QR-005').id, quantity: 2 }
    })

    await prisma.workOrderMessage.create({
        data: { workOrderId: wo3.id, userId: tech1.id, content: 'Przystępuję do wymiany paska. Przenośnik odłączony od zasilania.', sentAt: new Date('2026-04-01T08:20:00') }
    })
    ...
    const wo4 = await prisma.workOrder.create({
        data: {
            ...
            description: 'Minął termin okresowej wymiany filtrów. Filtr powietrza mocno zabrudzony.',
    ...
    const wo5 = await prisma.workOrder.create({
        data: {
            ...
            description: 'Maszyna zgłasza błąd E001 – brak sygnału z czujnika pozycji osi X.',
            priority: Priority.CRITICAL,
            status: WorkOrderStatus.NEW,
    ...
    const wo6 = await prisma.workOrder.create({
        data: {
            ...
            description: 'Planowy przegląd układu hydraulicznego po 500 godzinach pracy.',
        ...
        data: [
            {
                ...
                message: 'System CMMS został pomyślnie uruchomiony. Możesz rozpocząć pracę.',
                isRead: true,
            ...
            },
            {
                ...
                message: 'Część "O-ring NBR 50x3" spadła poniżej punktu zamówienia. Aktualny stan: 2 szt.',
                isRead: false,
                createdAt: new Date('2026-04-10T10:00:00')
            },
            {
                ...
                message: 'Zgłoszono awarię krytyczną maszyny CNC Fanuc 30i. Czujnik pozycji osi X nie odpowiada.',
                isRead: false,
            ...
            },
            {
                ...
                message: 'Za 7 dni upływa termin przeglądu układu hydraulicznego prasy H200.',
                isRead: false,
            ...
            },
            {
                ...
                message: 'Przypisano Ci zlecenie: "Wymiana paska napędowego" – przenośnik PT-50.',
                isRead: true,
            ...
            },
            {
                ...
                message: 'Certyfikat HEIGHT_WORK wygasa 2025-06-01. Skontaktuj się z działem HR.',
                isRead: false,
                createdAt: new Date('2026-04-15T06:00:00')
            }
        ]
