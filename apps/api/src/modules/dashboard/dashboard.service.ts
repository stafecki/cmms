...
const getCostStats = async (from: Date, to: Date): Promise<CostStats> => {
  ...
  const costByMachine: Record<
    string,
    { machineName: string; totalCost: number }
  > = {}
  ...
  for (const wo of completedOrders) {
    ...
    costByMachine[wo.machineId].totalCost +=
      Number(wo.laborCost) + Number(wo.partsCost)
  }
...
export const getDashboard = async (period: Period): Promise<DashboardData> => {
  ...
  const [workOrders, costs, machines, inventory, preventive] =
    await Promise.all([
